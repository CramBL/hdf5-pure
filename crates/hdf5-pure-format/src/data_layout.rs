...
impl DataLayout {
    ...
    fn parse_v3(
        ...
        match layout_class {
            ...
            LAYOUT_CLASS_CHUNKED => {
                ...
                bytes::ensure_len(data, p, os)?;
                let address =
                    bytes::read_optional_offset(data, p, offset_size)?.map(StoredAddress::new);
                p += os;
                // chunk dim sizes: dimensionality × 4 bytes each
                bytes::ensure_len(data, p, dimensionality * 4)?;
    ...
    fn parse_v4(
        ...
        match layout_class {
            ...
            LAYOUT_CLASS_CHUNKED => {
                ...
                // dimension sizes
                bytes::ensure_len(data, p, dimensionality * dim_size_encoded_length)?;
                let mut chunk_dimensions = Vec::with_capacity(dimensionality);
                for _ in 0..dimensionality {
    ...
    fn parse_compact(data: &[u8]) -> Result<DataLayout, FormatError> {
        ...
        bytes::ensure_len(data, COMPACT_DATA_OFFSET, data_size)?;
    ...
    fn parse_contiguous(
        ...
        let pos = LAYOUT_PROPERTIES_OFFSET;
        let os = offset_size as usize;
        bytes::ensure_len(data, pos, os + length_size as usize)?;
        Ok(DataLayout::Contiguous {
            address: bytes::read_optional_offset(data, pos, offset_size)?.map(StoredAddress::new),
            size: bytes::read_length(data, pos + os, length_size)?,
        })
    }
}
...
pub struct ChunkedLayoutFlags(u8);
...
impl ChunkedLayoutFlags {
    ...
    pub const fn partial_edge_chunks_stored_raw(self) -> bool {
...
impl ChunkIndexLayout {
    ...
    pub fn address(self) -> Option<StoredAddress> {
        match self {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct FilteredSingleChunk {
...
fn read_filtered_single_chunk(
    ...
    bytes::ensure_len(data, mask_at, FILTER_MASK_LEN)?;
    Ok(Some(FilteredSingleChunk {
        filtered_size: bytes::read_length(data, pos, length_size)?,
...
pub const COMPACT_DATA_OFFSET: usize = LAYOUT_PROPERTIES_OFFSET + 2;
...
pub const DONT_FILTER_PARTIAL_BOUND_CHUNKS: u8 = 0x01;
...
pub const SINGLE_INDEX_WITH_FILTER: u8 = 0x02;
