...
fn lzf_crosscheck() {
    ...
    for fix in &manifest.fixtures {
        ...
        if fix.filter_mask.unwrap_or(0) & 1 == 1 {
            ...
            let decoded = hdf5_pure_filter::decompress_lzf(&stored, Some(raw.len()))
        ...
        let ours = hdf5_pure_filter::compress_lzf(&raw);
        assert_eq!(
            hdf5_pure_filter::decompress_lzf(&ours, Some(raw.len())).unwrap(),
            raw,
