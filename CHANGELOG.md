...
- A read, an edit, an append or a copy on a malformed or missing input returns the error that describes what was wrong, in place of a fixed message string on `Error::EditUnsupported` or a variant that stood for another failure, with three new variants for the failures that had none of their own. A caller that matched on those message-carrying variants to detect a bad path or an unreadable header matches the failure itself instead, most often an `Error::Format`.
...
- A lookup or a staged write through a file or a group handle takes a whole object path, with a repeated or a trailing separator and a `.` component dropped and a leading `/` starting the path at the root group, where a group's lookup took one link name, a `.` component failed with `FormatError::PathNotFound`, and a write placed an absolute path below the group. An error reports the object by its path from the root group, except through a handle an object reference produced, which has no path of its own ([#567](https://github.com/CramBL/hdf5-pure/pull/567)).
- `Group::delete` and the group's staged creations reject a path that spells no link, `""`, `"."` or `"/"`, with `Error::EditUnsupported`, where a deletion removed that group and everything under it ([#567](https://github.com/CramBL/hdf5-pure/pull/567)).
